function [groundTruth_position,initial_position,output_position, spacial_uncert, error_location, ATE_result, accuracyArray] = bundle_adjustment_lessMeas(uncertainty, ATE, accuracy, ITERATIONS, percentage, windowSize)
...
%% Compute the number of maximal measurement for each features 
newNumbMeas = zeros(size(landmarks_used)) ;
currentNumber = zeros(size(landmarks_used)) ;
landmark_seen2 = zeros(size(landmarks_used)) ;

if percentage~=100
    disp('Determining the number of measurements maximal for each landmark')
    for i=1:NUM_FRAMES
        f = 1;
        while f < size(features_gt, 2) && features_gt(i,f) > 0
            landmarkID = features_gt(i,f) ;
            
            if landmark_seen2(landmarkID,1) == 0
                [whichFrames,~] = find(features_gt == landmarkID) ;
                
                % determine how many measurmeent there are in the system
                oldNumbMeas = size(whichFrames,1) ;
                
                % number of measurement wanted
                newNumbMeas(landmarkID,1) = floor(percentage/100 * oldNumbMeas) ;
                currentNumber(landmarkID,1) = 0 ;
                landmark_seen2(landmarkID,1) = 1 ;
            end
...
% we add the landmark using a window size to avoid adding all the
% measurement in the same part of the graph : a measurement can be add only
% 1 time in 10 frames 

% create the graph before
graph = NonlinearFactorGraph;
initialEstimate = Values;

for i=1:windowSize:NUM_FRAMES
    window_end = i + windowSize - 1;

    % used to look if the measurement has been seen in this window 
    landmarks_in_window = zeros(size(landmarks_used));

    % add cam poses, features and measurement within the window 
    for j=i:window_end
        fprintf('Adding frame %d to graph...\n', j)

        cam_pose = Pose3(Rot3.Quaternion(camera_gt_noisy(j,8), camera_gt_noisy(j,5), camera_gt_noisy(j,6), camera_gt_noisy(j,7)), ...
            Point3(camera_gt_noisy(j,2), camera_gt_noisy(j,3), camera_gt_noisy(j,4)));
        ...
        while f < size(features_gt, 2) && features_gt(j,f) > 0
            ...
            % Initialise the point near ground-truth if it's the first time
            % we saw it 
            ...
            % if the measurement hasn't been seen too much time and not in
            % this window 
            test = currentNumber(feature_id,1) < newNumbMeas(feature_id,1) ;
            if landmarks_in_window(feature_id,1) < 1 && test && landmarks_used(feature_id,1) == 1
                % ****************************** TODO ****************************
